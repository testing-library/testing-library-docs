...
1. Wait for the result of the operation in your test by using one of
   [the async utilities](/docs/dom-testing-library/api-async.mdx) like
   [wait](/docs/dom-testing-library/api-async.mdx#wait) or a
   [`find*` query](/docs/dom-testing-library/api-queries.mdx#findby). For example:
