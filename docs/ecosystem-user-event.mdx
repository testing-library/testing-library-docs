...
The `skipPointerEventsCheck` option can be passed to every event where there is
a check for `pointer-events`: [dblClick](#dblclickelement-eventinit-options),
[hover](#hoverelement-options), [unhover](#unhoverelement-options),
