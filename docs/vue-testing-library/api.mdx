...
See [Queries](dom-testing-library/api-queries.mdx) for a complete list of
available methods.
...
your own Element container via this option, it will not be appended to the
...
[firing events](dom-testing-library/api-events.mdx). However, it alters them so
that all events are async.
