...
## Differences from the Preact Testing Library
...
In extension of the original API, the render function of this testing library
supports a convenient `location` option that will set up an in-memory router
pointing at the specified location. Since this setup is not instantaneous, you
...
described later) has no `container`, `baseElement` or queries in its options or
return value. Instead, it has an `owner` to be used with
...
(string, HTMLElement or function returning a HTMLElement) in the `options` and
