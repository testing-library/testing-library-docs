...
[this guide on which query to use](guide-which-query.mdx) as well as the
...
:::caution Queries in Testcafe Testcafe has
...
for this reason, there's no difference between `queryBy` and `findBy` queries in
testcafe testing library. `getBy` queries will throw an exception (as designed)
so they will fail immediately and do not work with the built in waiting that
Testcafe provides. :::
...
:::caution The dom-testing-library umd must come before your configure script
:::
