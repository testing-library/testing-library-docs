...
Because of this, it's not a good idea to trigger mutations within the callback
as that may leave the document in an unpredictable state. Note that the number
of calls is constrained by the `timeout` and `interval` options.
